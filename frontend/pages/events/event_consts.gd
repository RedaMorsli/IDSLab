class_name EventConsts
extends GDScript

enum {EXP_FIELD_TYPE_1, EXP_FIELD_TYPE_2}

const ExpressionFields = {
	"uid": EXP_FIELD_TYPE_1,
	"pid": EXP_FIELD_TYPE_1,
	"mntns": EXP_FIELD_TYPE_2,
	"pidns": EXP_FIELD_TYPE_2,
	"tree": EXP_FIELD_TYPE_2,
	"uts": EXP_FIELD_TYPE_2,
	"comm": EXP_FIELD_TYPE_2,
	"container": EXP_FIELD_TYPE_2,
	"executable": EXP_FIELD_TYPE_2
}

const ExpressionType1Operators = [
	'=', '!=', '<', '>', '<=', '>='
]

const ExpressionType2Operators = [
	'=', '!='
]

const NetworkEventSet = 'network_events'

const NetworkEvents = [
	"net_flow_tcp_begin",
	"net_flow_tcp_end",
	"net_packet_ipv4",
	"net_packet_ipv6",
	"net_packet_tcp",
	"net_packet_udp",
	"net_packet_icmp",
	"net_packet_icmpv6",
	"net_packet_dns",
	"net_packet_dns_request",
	"net_packet_dns_response",
	"net_packet_http",
	"net_packet_http_request",
	"net_packet_http_response"
]

const SecurityEvents = [
	"anti_debugging",
	"aslr_inspection",
	"cgroup_notify_on_release",
	"cgroup_release_agent",
	"core_pattern_modification",
	"default_loader_mod",
	"disk_mount",
	"docker_abuse",
	"dropped_executable",
	"dynamic_code_loading",
	"fileless_execution",
	"hidden_file_created",
	"illegitimate_shell",
	"kernel_module_loading",
	"k8s_api_connection",
	"k8s_cert_theft",
	"ld_preload",
	"proc_fops_hooking",
	"proc_kcore_read",
	"proc_mem_access",
	"proc_mem_code_injection",
	"ptrace_code_injection",
	"process_vm_write_inject",
	"rcd_modification",
	"sched_debug_recon",
	"scheduled_task_mod",
	"stdio_over_socket",
	"sudoers_modification",
	"syscall_hooking",
    "system_request_key_mod"
]


const ContainerMetrics = {
	"container_blkio_device_usage_total": "counter",
	"container_cpu_cfs_periods_total": "counter",
	"container_cpu_cfs_throttled_periods_total": "counter",
	"container_cpu_cfs_throttled_seconds_total": "counter",
	"container_cpu_load_average_10s": "gauge",
	"container_cpu_schedstat_run_periods_total": "counter",
	"container_cpu_schedstat_runqueue_seconds_total": "counter",
	"container_cpu_schedstat_run_seconds_total": "counter",
	"container_cpu_system_seconds_total": "counter",
	"container_cpu_usage_seconds_total": "counter",
	"container_cpu_user_seconds_total": "counter",
	"container_file_descriptors": "gauge",
	"container_fs_inodes_free": "gauge",
	"container_fs_inodes_total": "gauge",
	"container_fs_io_current": "gauge",
	"container_fs_io_time_seconds_total": "counter",
	"container_fs_io_time_weighted_seconds_total": "counter",
	"container_fs_limit_bytes": "gauge",
	"container_fs_reads_bytes_total": "counter",
	"container_fs_read_seconds_total": "counter",
	"container_fs_reads_merged_total": "counter",
	"container_fs_reads_total": "counter",
	"container_fs_sector_reads_total": "counter",
	"container_fs_sector_writes_total": "counter",
	"container_fs_usage_bytes": "gauge",
	"container_fs_writes_bytes_total": "counter",
	"container_fs_write_seconds_total": "counter",
	"container_fs_writes_merged_total": "counter",
	"container_fs_writes_total": "counter",
	"container_hugetlb_failcnt": "counter",
	"container_hugetlb_max_usage_bytes": "gauge",
	"container_hugetlb_usage_bytes": "gauge",
	"container_last_seen": "gauge",
	"container_llc_occupancy_bytes": "gauge",
	"container_memory_bandwidth_bytes": "gauge",
	"container_memory_bandwidth_local_bytes": "gauge",
	"container_memory_cache": "gauge",
	"container_memory_failcnt": "counter",
	"container_memory_failures_total": "counter",
	"container_memory_mapped_file": "gauge",
	"container_memory_max_usage_bytes": "gauge",
	"container_memory_migrate": "gauge",
	"container_memory_numa_pages": "gauge",
	"container_memory_rss": "gauge",
	"container_memory_swap": "gauge",
	"container_memory_usage_bytes": "gauge",
	"container_memory_working_set_bytes": "gauge",
	"container_network_advance_tcp_stats_total": "gauge",
	"container_network_receive_bytes_total": "counter",
	"container_network_receive_errors_total": "counter",
	"container_network_receive_packets_dropped_total": "counter",
	"container_network_receive_packets_total": "counter",
	"container_network_tcp6_usage_total": "gauge",
	"container_network_tcp_usage_total": "gauge",
	"container_network_transmit_bytes_total": "counter",
	"container_network_transmit_errors_total": "counter",
	"container_network_transmit_packets_dropped_total": "counter",
	"container_network_transmit_packets_total": "counter",
	"container_network_udp6_usage_total": "gauge",
	"container_network_udp_usage_total": "gauge",
	"container_oom_events_total": "counter",
	"container_perf_events_scaling_ratio": "gauge",
	"container_perf_events_total": "counter",
	"container_perf_uncore_events_scaling_ratio": "gauge",
	"container_perf_uncore_events_total": "counter",
	"container_processes": "gauge",
	"container_referenced_bytes": "gauge",
	"container_sockets": "gauge",
	"container_spec_cpu_period": "gauge",
	"container_spec_cpu_quota": "gauge",
	"container_spec_cpu_shares": "gauge",
	"container_spec_memory_limit_bytes": "gauge",
	"container_spec_memory_reservation_limit_bytes": "gauge",
	"container_spec_memory_swap_limit_bytes": "gauge",
	"container_start_time_seconds": "gauge",
	"container_tasks_state": "gauge",
	"container_threads": "gauge",
	"container_threads_max": "gauge",
	"container_ulimits_soft": "gauge"
}


const SyscallEventSet = 'syscalls'

const SyscallEvents = [
	'accept',
	'accept4',
	'access',
	'acct',
	'add_key',
	'adjtimex',
	'afs',
	'afs_syscall',
	'alarm',
	'arch_prctl',
	'bdflush',
	'bind',
	'bpf',
	'break',
	'brk',
	'capget',
	'capset',
	'chdir',
	'chmod',
	'chown',
	'chown16',
	'chroot',
	'clock_adjtime',
	'clock_adjtime64',
	'clock_getres',
	'clock_getres_time32',
	'clock_gettime',
	'clock_gettime32',
	'clock_nanosleep',
	'clock_nanosleep_time32',
	'clock_settime',
	'clock_settime32',
	'clone',
	'clone3',
	'close',
	'close_range',
	'connect',
	'copy_file_range',
	'creat',
	'create_module',
	'delete_module',
	'dup',
	'dup2',
	'dup3',
	'epoll_create',
	'epoll_create1',
	'epoll_ctl',
	'epoll_ctl_old',
	'epoll_pwait',
	'epoll_pwait2',
	'epoll_wait',
	'epoll_wait_old',
	'eventfd',
	'eventfd2',
	'execve',
	'execveat',
	'exit',
	'exit_group',
	'faccessat',
	'faccessat2',
	'fadvise64',
	'fadvise64_64',
	'fallocate',
	'fanotify_init',
	'fanotify_mark',
	'fchdir',
	'fchmod',
	'fchmodat',
	'fchown',
	'fchown16',
	'fchownat',
	'fcntl',
	'fcntl64',
	'fdatasync',
	'fgetxattr',
	'finit_module',
	'flistxattr',
	'flock',
	'fork',
	'fremovexattr',
	'fsconfig',
	'fsetxattr',
	'fsmount',
	'fsopen',
	'fspick',
	'fstat',
	'fstat64',
	'fstatfs',
	'fstatfs64',
	'fsync',
	'ftime',
	'ftruncate',
	'ftruncate64',
	'futex',
	'futex_time32',
	'futimesat',
	'get_kernel_syms',
	'get_mempolicy',
	'get_robust_list',
	'get_thread_area',
	'getcpu',
	'getcwd',
	'getdents',
	'getdents64',
	'getegid',
	'getegid16',
	'geteuid',
	'geteuid16',
	'getgid',
	'getgid16',
	'getgroups',
	'getgroups16',
	'getitimer',
	'getpeername',
	'getpgid',
	'getpgrp',
	'getpid',
	'getpmsg',
	'getppid',
	'getpriority',
	'getrandom',
	'getresgid',
	'getresgid16',
	'getresuid',
	'getresuid16',
	'getrlimit',
	'getrusage',
	'getsid',
	'getsockname',
	'getsockopt',
	'gettid',
	'gettimeofday',
	'getuid',
	'getuid16',
	'getxattr',
	'gtty',
	'idle',
	'init_module',
	'inotify_add_watch',
	'inotify_init',
	'inotify_init1',
	'inotify_rm_watch',
	'io_cancel',
	'io_destroy',
	'io_getevents',
	'io_pgetevents',
	'io_pgetevents_time32',
	'io_setup',
	'io_submit',
	'io_uring_enter',
	'io_uring_register',
	'io_uring_setup',
	'ioctl',
	'ioperm',
	'iopl',
	'ioprio_get',
	'ioprio_set',
	'ipc',
	'kcmp',
	'kexec_file_load',
	'kexec_load',
	'keyctl',
	'kill',
	'landlock_add_rule',
	'landlock_create_ruleset',
	'landlock_restrict_self',
	'lchown',
	'lchown16',
	'lgetxattr',
	'link',
	'linkat',
	'listen',
	'listxattr',
	'llistxattr',
	'llseek',
	'lock',
	'lookup_dcookie',
	'lremovexattr',
	'lseek',
	'lsetxattr',
	'lstat',
	'lstat64',
	'madvise',
	'mbind',
	'membarrier',
	'memfd_create',
	'memfd_secret',
	'migrate_pages',
	'mincore',
	'mkdir',
	'mkdirat',
	'mknod',
	'mknodat',
	'mlock',
	'mlock2',
	'mlockall',
	'mmap',
	'mmap2',
	'modify_ldt',
	'mount',
	'mount_setattr',
	'move_mount',
	'move_pages',
	'mprotect',
	'mpx',
	'mq_getsetattr',
	'mq_notify',
	'mq_open',
	'mq_timedreceive',
	'mq_timedreceive_time32',
	'mq_timedsend',
	'mq_timedsend_time32',
	'mq_unlink',
	'mremap',
	'msgctl',
	'msgget',
	'msgrcv',
	'msgsnd',
	'msync',
	'munlock',
	'munlockall',
	'munmap',
	'name_to_handle_at',
	'nanosleep',
	'newfstatat',
	'nfsservctl',
	'nice',
	'old_getrlimit',
	'old_select',
	'oldfstat',
	'oldlstat',
	'oldolduname',
	'oldstat',
	'olduname',
	'open',
	'open_by_handle_at',
	'open_tree',
	'openat',
	'openat2',
	'pause',
	'perf_event_open',
	'personality',
	'pidfd_getfd',
	'pidfd_open',
	'pidfd_send_signal',
	'pipe',
	'pipe2',
	'pivot_root',
	'pkey_alloc',
	'pkey_free',
	'pkey_mprotect',
	'poll',
	'ppoll',
	'ppoll_time32',
	'prctl',
	'pread64',
	'preadv',
	'preadv2',
	'prlimit64',
	'process_madvise',
	'process_mrelease',
	'process_vm_readv',
	'process_vm_writev',
	'prof',
	'profil',
	'pselect6',
	'pselect6_time32',
	'ptrace',
	'putpmsg',
	'pwrite64',
	'pwritev',
	'pwritev2',
	'query_module',
	'quotactl',
	'quotactl_fd',
	'read',
	'readahead',
	'readdir',
	'readlink',
	'readlinkat',
	'readv',
	'reboot',
	'recvfrom',
	'recvmmsg',
	'recvmmsg_time32',
	'recvmsg',
	'remap_file_pages',
	'removexattr',
	'rename',
	'renameat',
	'renameat2',
	'request_key',
	'restart_syscall',
	'rmdir',
	'rseq',
	'rt_sigaction',
	'rt_sigpending',
	'rt_sigprocmask',
	'rt_sigqueueinfo',
	'rt_sigreturn',
	'rt_sigsuspend',
	'rt_sigtimedwait',
	'rt_sigtimedwait_time32',
	'rt_tgsigqueueinfo',
	'sched_get_priority_max',
	'sched_get_priority_min',
	'sched_getaffinity',
	'sched_getattr',
	'sched_getparam',
	'sched_getscheduler',
	'sched_rr_get_interval',
	'sched_rr_get_interval_time32',
	'sched_setaffinity',
	'sched_setattr',
	'sched_setparam',
	'sched_setscheduler',
	'sched_yield',
	'seccomp',
	'security',
	'select',
	'semctl',
	'semget',
	'semop',
	'semtimedop',
	'sendfile',
	'sendfile32',
	'sendmmsg',
	'sendmsg',
	'sendto',
	'set_mempolicy',
	'set_robust_list',
	'set_thread_area',
	'set_tid_address',
	'setdomainname',
	'setfsgid',
	'setfsgid16',
	'setfsuid',
	'setfsuid16',
	'setgid',
	'setgid16',
	'setgroups',
	'setgroups16',
	'sethostname',
	'setitimer',
	'setns',
	'setpgid',
	'setpriority',
	'setregid',
	'setregid16',
	'setresgid',
	'setresgid16',
	'setresuid',
	'setresuid16',
	'setreuid',
	'setreuid16',
	'setrlimit',
	'setsid',
	'setsockopt',
	'settimeofday',
	'setuid',
	'setuid16',
	'setxattr',
	'sgetmask',
	'shmat',
	'shmctl',
	'shmdt',
	'shmget',
	'shutdown',
	'sigaction',
	'sigaltstack',
	'signal',
	'signalfd',
	'signalfd4',
	'sigpending',
	'sigprocmask',
	'sigreturn',
	'sigsuspend',
	'socket',
	'socketcall',
	'socketpair',
	'splice',
	'ssetmask',
	'stat',
	'stat64',
	'statfs',
	'statfs64',
	'statx',
	'stime',
	'stty',
	'swapoff',
	'swapon',
	'symlink',
	'symlinkat',
	'sync',
	'sync_file_range',
	'syncfs',
	'sysctl',
	'sysfs',
	'sysinfo',
	'syslog',
	'tee',
	'tgkill',
	'time',
	'timer_create',
	'timer_delete',
	'timer_getoverrun',
	'timer_gettime',
	'timer_gettime32',
	'timer_settime',
	'timer_settime32',
	'timerfd_create',
	'timerfd_gettime',
	'timerfd_gettime32',
	'timerfd_settime',
	'timerfd_settime32',
	'times',
	'tkill',
	'truncate',
	'truncate64',
	'tuxcall',
	'ulimit',
	'umask',
	'umount',
	'umount2',
	'uname',
	'unlink',
	'unlinkat',
	'unshare',
	'uselib',
	'userfaultfd',
	'ustat',
	'utime',
	'utimensat',
	'utimensat_time32',
	'utimes',
	'vfork',
	'vhangup',
	'vm86',
	'vm86old',
	'vmsplice',
	'vserver',
	'wait4',
	'waitid',
	'waitpid',
	'write',
	'writev',
]
